...
def sort_transactions(file, start, end):
    ...
    if sum(1 if isinstance(directive, models.Transaction) else 0 for directive in file.directives[start:end]) > 1:
        file.directives[start:end] = [copy.deepcopy(d) for d in sorted(file.directives[start:end], key=sortkey)]

parser = Parser()
file = parser.parse(open(sys.argv[1]).read(), models.File)
start_range = None
for i, directive in enumerate(file.directives):
    if isinstance(directive, models.Transaction) or isinstance(directive, models.IgnoredLine) or isinstance(directive, models.Balance):
