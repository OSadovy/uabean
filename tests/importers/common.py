...
def run_importer_test(importer, capsys):
    _run(Context([importer]), [os.path.abspath(f"tests/importers/{importer.account('')}/")], "", 0, 0)
    captured = capsys.readouterr()
    assert "PASSED" in captured.out
    assert "ERROR"  not in captured.out

def run_importer_test_with_existing_entries(importer, document, existing_entries_filename):
    ...
    existing_entries = loader.load_file(os.path.join(base_path, existing_entries_filename))[0]
